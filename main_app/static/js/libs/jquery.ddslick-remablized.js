// IMPORTANT - before updating this file, see the minor addition of line 81 which is as follows:
//  if($this.is(':selected')) { options.defaultSelectedIndex = index }.
//  The addition includes the 'index' addition as as arguement to the anonymous function - James
...
(function ($) {
        ...
        return this.each(function () {
            ...
            if (!data) {
                ...
                obj.find('option').each(function (index) {
                    ...
                    // this shouldn't be necessary, but DDslick currently has an issue acknowleging
                    // a selected option when using the HTML option
                    if($this.is(':selected') && options.defaultSelectedIndex == null) { options.defaultSelectedIndex = index }
                });
                ...
                $(document).keydown(function(e) {
                    if (enableKeys == true && options.enableKeyboard == true) {
                        ...
                        if (e.keyCode == options.keyboard[0].down) {
                            if (0 <= curIndex && curIndex <= ddOptionsLen) {
                              ddOptions.find("li").eq(curIndex).find('a').removeClass('dd-option-hover');
                              curIndex <= ddOptionsLen ? curIndex++ : curIndex = 0;
                              ddHighlighted = ddOptions.find("li").eq(curIndex).find('a').addClass('dd-option-hover');
                              scrollDownOption(ddOptions, ddHighlighted);
                            }
                        }else if (e.keyCode == options.keyboard[0].up) {
                            if (0 < curIndex && curIndex <= ddOptionsLen + 1) {
                              ddOptions.find("li").eq(curIndex).find('a').removeClass('dd-option-hover');
                              curIndex >= 0 ? curIndex-- : curIndex = ddOptionsLen;
                              ddHighlighted = ddOptions.find("li").eq(curIndex).find('a').addClass('dd-option-hover');
                              scrollUpOption(ddOptions, ddHighlighted);
                            }
    ...
    function selectIndex(obj, index, skipAnimation) {
    ...
    function open(obj) {
        if (!obj.hasClass('dd-disabled')){
            ...
            var ddSelected = obj.find('.dd-option-selected');
            ...
            if (wasOpen) {
                ...
                ddOptions.slideDown('fast', function() {
                  if(ddSelected.length != 0) { scrollCurrentOption(ddOptions, ddSelected); }
                });
                ddPointer.addClass('dd-pointer-up');
                obj.addClass('dd-expanded');
            }
    ...
    //Private: Option scrolling
    function scrollCurrentOption(ddOptions, ddOption) {
      var scrollPosition = ddOptions.scrollTop() + ddOption.offset().top - ddOptions.offset().top;
      ddOptions.animate({scrollTop: scrollPosition});
    }

    function scrollUpOption(ddOptions, ddOption) {
      var scrollPosition = ddOptions.scrollTop() + ddOption.offset().top - ddOptions.offset().top;

      if (ddOption.offset().top < ddOptions.offset().top) {
        ddOptions.scrollTop(scrollPosition);
      }
    }

    function scrollDownOption(ddOptions, ddOption) {
      var offset = ddOption.offset().top - ddOptions.offset().top + ddOption.outerHeight();

      if (offset > ddOptions[0].clientHeight) {
        ddOptions.scrollTop(ddOptions.scrollTop() + offset - ddOptions[0].clientHeight);
      }
    }

    //Private: Close the drop down options
    function close(obj, skipAnimation) {
